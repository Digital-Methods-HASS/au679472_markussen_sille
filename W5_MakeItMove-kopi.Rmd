...
Log-scaling the axis makes big differences relatively smaller. Since there is one outlier with a much higher gdp than the other values it makes sense to have a log10 scale which makes it easier to discern the distribution of the remaining values and to interpret the differences.
...
```
The richest country in 1952 is Kuwait with a GDP per capita of 108382.4
...
ggplot(subset(gapminder, year == 2007), aes(gdpPercap, lifeExp, size = pop, colour = continent)) +
  ...
  labs(x = "GDP (per capita)", y = "Life Expectancy", title = "Differentiated by continent")+
  scale_x_log10()
  options(scipen = 999)
```
...
How does the the relationship between life expectancy and GDP change over time across continents?
...
anim2 <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop)) +
  ...
  scale_x_log10() + # convert x to log scale
  transition_time(year)+
  facet_wrap(~continent) + # Using facet_wrap to create separate plots for each continent
  labs(title = "Year: {frame_time}", y = "Life Expectancy", x = "GDP (per capita)")
  options(scipen = 999)
anim2
